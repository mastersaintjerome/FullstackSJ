...
export function retrieveError(retrieveError) {
  return { type: 'ROLE_UPDATE_RETRIEVE_ERROR', retrieveError };
}

export function retrieveLoading(retrieveLoading) {
  return { type: 'ROLE_UPDATE_RETRIEVE_LOADING', retrieveLoading };
}

export function retrieveSuccess(retrieved) {
  return { type: 'ROLE_UPDATE_RETRIEVE_SUCCESS', retrieved };
}
...
export function updateError(updateError) {
  return { type: 'ROLE_UPDATE_UPDATE_ERROR', updateError };
}

export function updateLoading(updateLoading) {
  return { type: 'ROLE_UPDATE_UPDATE_LOADING', updateLoading };
}

export function updateSuccess(updated) {
  return { type: 'ROLE_UPDATE_UPDATE_SUCCESS', updated };
}
...
export function mercureOpen(eventSource) {
  return { type: 'ROLE_UPDATE_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'ROLE_UPDATE_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'ROLE_UPDATE_MERCURE_MESSAGE', retrieved });
  };
}
